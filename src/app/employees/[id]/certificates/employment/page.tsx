...
export default function EmploymentCertificatePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6 no-print">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">재직증명서</h1>
        </div>
      ...
      <Card className="max-w-2xl mx-auto print-area">
        <CardContent className="p-10">
          ...
          {/* Employment Info */}
          <div className="space-y-4 mb-10">
            <h2 className="text-lg font-semibold border-b pb-2 mb-4">재직사항</h2>
            <div className="grid grid-cols-[120px_1fr] gap-y-3 text-sm">
              ...
              <span className="text-muted-foreground">직 책</span>
              <span className="font-medium">{employee.position_title?.name ?? '-'}</span>
              ...
              <span className="font-medium">{employee.employment_type === 'regular' ? '정규직' : '계약직'}</span>
            </div>
          </div>

          {/* Purpose */}
          <div className="text-center my-12 text-base leading-relaxed">
            <p>위와 같이 재직하고 있음을 증명합니다.</p>
          </div>
